...
import { Play, ListMusic, Plus, Loader2, ArrowLeft, Clock, Music } from "lucide-react";
...
export default function PlaylistsView({
  ...
  const playPlaylist = (playlist: PlaylistWithSongs, startSong?: Song) => {
    ...
    const startIndex = startSong ? playlist.songs.findIndex(s => s.id === startSong.id) : 0;
      ...
      <div className="flex flex-col gap-6 mt-8">
          ...
          <div>
            ...
            <div className="flex items-center gap-4 text-sm text-zinc-500 mt-1">
              ...
              <span className="flex items-center gap-1">
                ...
                Last updated: {selectedPlaylist.updatedAt ? new Date(selectedPlaylist.updatedAt).toLocaleDateString() : 'Never'}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col divide-y divide-zinc-800/50 mt-4">
          {selectedPlaylist.songs.length === 0 ? (
            <p className="text-zinc-500 text-center py-12">This playlist is empty.</p>
          ) : (
